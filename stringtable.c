...
stringtable_ref 
new_stringtable(void) {
    stringtable_ref new_table;
    if ((new_table = 
        (stringtable_ref)malloc(sizeof(struct stringtable))) == NULL) {
        return NULL;
    }
    ...
    if ((new_table->table_head = 
        (stringnode_ref *)calloc(new_table->dimension, sizeof(stringnode_ref))) == NULL) {
        return NULL;
    }
...
inline uint32_t 
nearest_power_two(uint32_t num) {
...
void 
debugdump_stringtable(stringtable_ref table, FILE* log) {
    if(!table)
        return;
...
void 
debugdump_node_chain(stringnode_ref chain_head, FILE * log, unsigned int table_offset) {
    if(!chain_head)
        return;
    ...
    fprintf(log, "%8d  %12u  \"%s\"\n", 
            table_offset, chain_head->node_hash, 
            chain_head->node_string);
    mover = mover->next;
    while(mover != NULL) {
        fprintf(log, "          %12u  \"%s\"\n", 
...
stringnode_ref 
intern_stringtable(stringtable_ref table, cstring string) {
    if(!table)
        return NULL;
    ...
    stringnode_ref intern_node = 
        find_node(head, dimension, string_hash);
    ...
    if(!intern_node) {
        ...
        if(head[string_hash % dimension] == NULL) {
            ...
            stringnode_ref chain_tail = 
                find_chain_tail(head[string_hash % dimension]);
    ...
    if(!(table->ops_count % REHASH_OPS_THRESHOLD)) {
        ...
        if(nloadf >= LOAD_MAX) {
            rehash_table(table, 
                         nearest_power_two(table->node_count * 2) - 1);
        }
    }
    return intern_node;
}
...
cstring 
peek_stringtable(stringnode_ref node) {
    return node->node_string;
}
cstring 
repr_stringnode(stringnode_ref node) {
    cstring repr_string = 
        (cstring)calloc(strlen(node->node_string) + 14, sizeof(char));
    sprintf(repr_string, "%s %12d", node->node_string, node->node_hash);
    return repr_string;
}
hashcode_t 
hashcode_stringtable(stringnode_ref node) {
    return node->node_hash;
}

void 
rehash_table(stringtable_ref table, size_t new_size) {
    stringnode_ref *new_table = 
        (stringnode_ref *)calloc(new_size, sizeof(stringnode_ref));
    unsigned int i;
    ...
    for(i = 0; i < table->dimension; i++) {
        ...
        for(; mover != NULL; 
            mover = next, next = mover == NULL ? NULL : mover->next) {
            mover->prev = NULL;
            mover->next = NULL;

            uint32_t new_offset = mover->node_hash % new_size;
            DEBUGF('a', "NODE: %p OLD_LOC %d NEW_LOC %d\n", 
                    mover, mover->node_hash % table->dimension, 
                    new_offset);
            ...
            if(new_table[new_offset] == NULL) {
                ...
                stringnode_ref chain_tail = 
                    find_chain_tail(new_table[new_offset]);
...
stringnode_ref find_chain_tail(stringnode_ref chain) {
    ...
    for(tail = chain, mover = chain->next; mover != NULL; 
        tail = mover, mover = mover->next);
    return tail;
}

stringnode_ref 
find_node(stringnode_ref *head, uint32_t dimension, hashcode_t hash) {
    return find_node_from_chain(head[hash % dimension], hash);
}

stringnode_ref 
find_node_from_chain(stringnode_ref chain, hashcode_t hash) {
    if(!chain)
        return NULL;
...
void 
delete_stringtable(stringtable_ref table) {
    if(!table)
        return;
...
void 
delete_node_chain(stringnode_ref node) {
    stringnode_ref mover, tail;
    if(!node)
        return;

    for(mover = node->next, tail = node; mover != NULL; 
        tail = mover, mover = mover->next) {
...
void 
delete_node(stringnode_ref node) {
